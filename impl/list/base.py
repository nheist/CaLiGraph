...
def get_listpage_entities(graph, listpage_uri: str) -> dict:
    ...
    if '__LISTPAGE_ENTITIES__' not in globals():
        __LISTPAGE_ENTITIES__ = defaultdict(dict, utils.load_or_create_cache('dbpedia_listpage_entities', lambda: _extract_listpage_entities(graph)))
...
def _extract_listpage_entities(graph):
    ...
    utils.get_logger().info('LIST/BASE: Extracting new entities from list pages..')

    utils.get_logger().info('LIST/BASE: Extracting enum entities..')
    enum_features = get_enum_listpage_entity_features(graph)
    enum_entities = list_extract.extract_enum_entities(enum_features)

    utils.get_logger().info('LIST/BASE: Extracting table entities..')
...
def get_enum_listpage_entity_features(graph) -> pd.DataFrame:
    ...
    if '__ENUM_LISTPAGE_ENTITY_FEATURES__' not in globals():
        __ENUM_LISTPAGE_ENTITY_FEATURES__ = utils.load_or_create_cache('dbpedia_listpage_enum_features', lambda: _compute_listpage_entity_features(graph, wikipedia.ARTICLE_TYPE_ENUM))
...
def get_table_listpage_entity_features(graph) -> pd.DataFrame:
    ...
    if '__TABLE_LISTPAGE_ENTITY_FEATURES__' not in globals():
        __TABLE_LISTPAGE_ENTITY_FEATURES__ = utils.load_or_create_cache('dbpedia_listpage_table_features', lambda: _compute_listpage_entity_features(graph, wikipedia.ARTICLE_TYPE_TABLE))
...
def _compute_listpage_entity_features(graph, list_type: str) -> pd.DataFrame:
    ...
    utils.get_logger().info(f'LIST/BASE: Computing entity features for {list_type}..')
    ...
    utils.get_logger().info('LIST/BASE: Assigning entity labels..')
