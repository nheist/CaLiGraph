...
def _retrieve_training_data(graph) -> tuple:
    ...
    with mp.Pool(processes=3) as pool:
        ...
        for token_lists, label_lists in tqdm(pool.imap_unordered(tokenize.page_to_tokens_and_labels, ctx, chunksize=300), desc='Extracting BERT training data', total=len(ctx)):
