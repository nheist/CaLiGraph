    ...
    for node in graph.traverse_nodes_topdown():
        for child in graph.children(node):
            if head_subject_lemmas[node] == head_subject_lemmas[child]:
                continue
            parent_disjoint_types = {dt for t in direct_mappings[node] for dt in dbp_store.get_all_disjoint_types(t)}
...
def resolve_disjointnesses(graph):
    ...
    for node in graph.traverse_nodes_topdown():
        ...
        if len(coherent_type_sets) > 1:
            ...
            direct_types = (direct_types | part_types).difference({dt for t in part_types for dt in dbp_store.get_all_disjoint_types(t)})
...
def _find_dbpedia_parents(graph, node: str, direct_resources_only: bool) -> dict:
    ...
    result = defaultdict(float, {t: score for t, score in result.items() if not dbp_store.get_all_disjoint_types(t).intersection(set(result))})
    return result
...
def _find_coherent_type_sets(dbp_types: dict) -> list:
    ...
    coherent_sets = []
    disjoint_type_mapping = {t: set(dbp_types).intersection(dbp_store.get_all_disjoint_types(t)) for t in dbp_types}
