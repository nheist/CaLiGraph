...
def run_prediction(model_name: str, sample: int, epochs: int, batch_size: int, loss: str, learning_rate: float, warmup_steps: int, weight_decay: float, num_ents: int, ent_dim: int, items_per_chunk: int, cls_predictor: bool, include_source_page: bool):
    run_id = f'EP_{model_name}_s-{sample}_ipc-{items_per_chunk}_ne-{num_ents}_isp-{include_source_page}_cp-{cls_predictor}_ed-{ent_dim}_e-{epochs}_bs-{batch_size}_loss-{loss}_lr-{learning_rate}_ws-{warmup_steps}_wd-{weight_decay}'
    ...
    # load data
    dataset_version = f'EP-{model_name}-s{sample}-ipc{items_per_chunk}-ne{num_ents}'
