...
def run_prediction(version: str, model_name: str, sample: int, epochs: int, batch_size: int, learning_rate: float, warmup_steps: int, weight_decay: float, num_ents: int, ent_dim: int, items_per_chunk: int, cls_predictor: bool, include_source_page: bool):
    run_id = f'MEMv{version}_{model_name}_s-{sample}_ipc-{items_per_chunk}_ne-{num_ents}_isp-{include_source_page}_cp-{cls_predictor}_ed-{ent_dim}_e-{epochs}_bs-{batch_size}-{learning_rate}_ws-{warmup_steps}_wd-{weight_decay}'
    ...
    # load data
    dataset_version = f'MEMv{version}-{model_name}-s{sample}-ipc{items_per_chunk}-ne{num_ents}'
...
def _load_train_and_val_datasets(tokenizer, sample: int, items_per_chunk: int, num_ents: int) -> Tuple[MentionEntityMatchingDataset, MentionEntityMatchingDataset]:
    train_data, val_data = prepare.get_train_and_val_data(sample, items_per_chunk)
