...
def run_prediction(model_name: str, sample: int, epochs: int, batch_size: int, learning_rate: float, warmup_steps: int, weight_decay: float, num_ents: int, ent_dim: int, items_per_chunk: int, cls_predictor: bool, include_source_page: bool):
    run_id = f'MEM_{model_name}_s-{sample}_ipc-{items_per_chunk}_ne-{num_ents}_isp-{include_source_page}_cp-{cls_predictor}_ed-{ent_dim}_e-{epochs}_bs-{batch_size}-{learning_rate}_ws-{warmup_steps}_wd-{weight_decay}'
    ...
    model = TransformerForMentionEntityMatching(encoder, include_source_page, cls_predictor, ent_idx2emb, ent_dim)
    # load data
...
def _load_train_and_val_datasets(tokenizer, sample: int, items_per_chunk: int, num_ents: int) -> Tuple[Dict[DbpResource, tuple], Dict[DbpResource, tuple]]:
    ...
    train_data = prepare_dataset(train_data, tokenizer, num_ents, items_per_chunk)
    ...
    val_data = prepare_dataset(val_data, tokenizer, num_ents, items_per_chunk)
