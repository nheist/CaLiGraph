...
def run_prediction(model_name: str, sample: int, epochs: int, batch_size: int, learning_rate: float, warmup_steps: int, weight_decay: float, num_ents: int, ent_dim: int, items_per_chunk: int, cls_predictor: bool):
    run_id = f'MEM_{model_name}_s-{sample}_ipc-{items_per_chunk}_ne-{num_ents}_cp-{cls_predictor}_ed-{ent_dim}_e-{epochs}_bs-{batch_size}-{learning_rate}_ws-{warmup_steps}_wd-{weight_decay}'
    ...
    model = TransformerForMentionEntityMatching(encoder, cls_predictor, ent_idx2emb, ent_dim, num_ents)
    # load data
