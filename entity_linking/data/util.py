...
class CandidateAlignment:
    def __init__(self, clustering: List[Tuple[Set[MentionId], Optional[int]]] = None):
    ...
    def _get_candidates(self, target_type, include_score: bool, nil_flag: Optional[bool]) -> Iterable[Union[Pair, Tuple[Pair, float]]]:
        for m_id, targets in self.mention_to_target_mapping.items():
            for t_id, score in targets.items():
                pair = (m_id, t_id)
                if isinstance(t_id, target_type) and is_consistent_with_nil_flag(pair, nil_flag):
                    yield (pair, score) if include_score else pair


def is_consistent_with_nil_flag(pair: Pair, nil_flag: Optional[bool]) -> bool:
    if nil_flag is None:
        return True
    if nil_flag:  # True, if any mention id is nil
        item_a, item_b = pair
        return is_nil_mention(item_a) or isinstance(item_b, MentionId) and is_nil_mention(item_b)
    else:  # True, if any mention id is not nil
        item_a, item_b = pair
        return not is_nil_mention(item_a) or isinstance(item_b, MentionId) and not is_nil_mention(item_b)


def is_nil_mention(mention_id: MentionId) -> bool:
