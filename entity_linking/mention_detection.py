...
def run_evaluation(model_name: str, epochs: int, batch_size: int, learning_rate: float, warmup_steps: int, weight_decay: float, ignore_tags: bool, predict_single_tag: bool, negative_sample_size: float, single_item_chunks: bool, train_on_listpages: bool, train_on_pages: bool):
    run_id = f'v3_{model_name}_tlp-{train_on_pages}_tp-{train_on_pages}_it-{ignore_tags}_st-{predict_single_tag}_nss-{negative_sample_size}_sic-{single_item_chunks}_e-{epochs}_lr-{learning_rate}_ws-{warmup_steps}_wd-{weight_decay}'
