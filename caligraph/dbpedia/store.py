...
def _get_supertype_mapping() -> pd.DataFrame:
    if '__SUPERTYPE_MAPPING__' not in globals():
        ...
        __SUPERTYPE_MAPPING__.apply(lambda row: __SUPERTYPE_MAPPING__[__SUPERTYPE_MAPPING__.index.isin(get_equivalent_types(set(row.index)))].any(), axis=1, result_type='broadcast')
