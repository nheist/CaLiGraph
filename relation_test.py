...
def get_property_value_count(property_tuples) -> dict:
...
for idx, cat in enumerate(categories):
    ...
    cat_property_value_count = get_property_value_count(cat_property_count.keys())
    ...
    if idx % 1000 == 0:
        print('=' * 20)
        print('Processed {} of {} categories in {}.'.format(idx, len(categories), (
                    datetime.datetime.now().replace(microsecond=0) - starttime)))
        print('FOUND RELATIONS FOR -> Categories: {} -- Properties: {} -- Instances: {}'.format(cat_counter,
                                                                                                property_counter,
                                                                                                instance_counter))

print(f'CATS: {cat_counter} -- PROPERTIES: {property_counter} -- INSTANCES: {instance_counter}')